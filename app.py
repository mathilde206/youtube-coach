...
def get_mongo_search_query(value, search_query):
    """
    This function takes the search criteria selected by the user and creates the appropriate
...
def get_videos(category, page_number):
    """
    This function will show the videos with and without filtering, by creating a query_filter
    string that will be used to retrieve the right videos.
...
def update_video(video_id):
    ...
    if mongo.db.videos.find_one({"_id": id_input}):
        mongo.db.videos.update({"_id": id_input}, {
            "$set": {
                "video_url": request.form.get("video_url"),
                "video_description": request.form.get("video_description"),
                "category_name": request.form.get("category_name"),
