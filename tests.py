...
class Test_Fitness_Vid(unittest.TestCase):
    ...
    def test_get_mongo_search_query(self):
        self.assertEqual(get_mongo_search_query('category_name', ["yoga"]),
            {'category_name': 'yoga'})
        self.assertEqual(get_mongo_search_query('category_name', ["yoga", "cardio"]),
            {'$or': [{'category_name': 'yoga'}, {'category_name': 'cardio'}]})
        self.assertEqual(get_mongo_search_query('category_name', ["yoga", "cardio", 'strenght']), {
            '$or': [{'category_name': 'yoga'}, {'category_name': 'cardio'},
                {'category_name': 'strenght'}]
        })
